...
class ChartAPIController extends Controller
{
    ...
    public function generateRandomList($length, $min_val, $max_val) {
        $randomList = [];
        
        for ($i = 0; $i < $length; $i++) {
            $randomList[] = rand($min_val, $max_val);
        }
    ...
    public function getData(Request $request){
        ...
        $distributionType = $request->header('Distribution-Type');
        $areaType = $request->header('Area-Type');
        $length = 5;
        $min_val = 500; 
        $max_val = 2000;
                    ...
                    case 'District':
                        return response()->json([
                            ...
                            'datasets'=>[
                                [
                                    'label'=>'North Goa',
                                    'data'=>$this->generateRandomList($length, $min_val, $max_val),
                                ],
                                [
                                    'label'=>'South Goa',
                                    'data'=>$this->generateRandomList($length, $min_val, $max_val),
            ...
            case '2':
                return response()->json([
                    ...
                    'datasets'=>[
                        [
                            'label'=>'Aadhar Seeded',
                            'data'=>$this->generateRandomList($length, 3000, 8000),
                        ],
                        [
                            'label'=>'Non-Aadhar Seeded',
                            'data'=>$this->generateRandomList($length, 3000, 8000),
                        ],
                    ]
                ]);
            ...
            case '5':
                return response()->json([
                    ...
                    'datasets'=>[
                        [
                            'label'=>'Beneficaries',
                            'data'=>$this->generateRandomList($length, 8000, 11000),
                        ],
                    ]
                ]);
